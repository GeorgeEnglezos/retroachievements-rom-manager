...
Native RetroAchievements hashing via rcheevos + libchdr, exposed over FFI.
  ...
  # (../src/raw_hash_apple_unity.c), which in turn compiles the full rcheevos +
  # libchdr tree. CocoaPods cannot list those ../src sources directly, so the
  # forwarder pulls them in and the search paths below let the compiler find
  # their headers. This mirrors ../src/CMakeLists.txt for the other platforms.
  s.source           = { :path => '.' }
